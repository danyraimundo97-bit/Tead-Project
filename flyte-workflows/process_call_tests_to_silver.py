...
def process_call_tests_to_silver(minio_access: str, minio_secret: str, target_date_str: str) -> str:
    try:
        ...
        # We added .str.replace('/', '_') right before .str.lower()
        ...
        df['date_of_test'] = pd.to_datetime(df['date_of_test'])
        daily_df = df[df['date_of_test'].dt.strftime('%Y-%m-%d') == target_date_str]
        ...
        cur.execute(f"""
            CREATE TABLE IF NOT EXISTS hive.staging.temp_tests_{target_date_str.replace('-','')} (
        ...
        # --- FIX 2: CORRECT CALL TESTS SCHEMA ---
        cur.execute("""
        ...
        cur.execute(f"""
            ...
            SELECT * FROM hive.staging.temp_tests_{target_date_str.replace('-','')}
        """)
        cur.fetchall()
        
        cur.execute(f"DROP TABLE hive.staging.temp_tests_{target_date_str.replace('-','')}")
        cur.fetchall()
